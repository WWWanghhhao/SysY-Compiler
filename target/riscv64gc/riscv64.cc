...
Register RiscV64Spiller::GenerateReadCode(std::list<MachineBaseInstruction *>::iterator &it, int raw_stk_offset,
    ...
    if (type == INT64) {
        auto ld_inst = rvconstructor->ConstructIImm(RISCV_LD, read_mid_reg, GetPhysicalReg(RISCV_sp), offset);
        cur_block->insert(it, ld_inst);
    } else if (type == FLOAT64) {
        auto ld_inst = rvconstructor->ConstructIImm(RISCV_FLD, read_mid_reg, GetPhysicalReg(RISCV_sp), offset);
        cur_block->insert(it, ld_inst);
    }
...
Register RiscV64Spiller::GenerateWriteCode(std::list<MachineBaseInstruction *>::iterator &it, int raw_stk_offset,
    ...
    ++it;
    if (type == INT64) {
        auto sd_inst = rvconstructor->ConstructSImm(RISCV_SD, write_mid_reg, GetPhysicalReg(RISCV_sp), offset);
        cur_block->insert(it, sd_inst);
    } else if (type == FLOAT64) {
        auto sd_inst = rvconstructor->ConstructSImm(RISCV_FSD, write_mid_reg, GetPhysicalReg(RISCV_sp), offset);
        cur_block->insert(it, sd_inst);
    }
    --it;
