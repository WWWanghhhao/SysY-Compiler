...
public:
    ...
    // 用于获取指令使用了哪些操作数
    virtual std::vector<Operand> GetUses() = 0;
    virtual Operand GetModified() = 0;
};
...
public:
    ...
    std::vector<Operand> GetUses() {
        std::vector<Operand> x;
        x.push_back(pointer);
        return x;
    }
    Operand GetModified() { return result; }
};
...
public:
    ...
    Operand GetModified() { return pointer; }
};
...
public:
    ...
    void ReplaceRegByMap(const std::map<int, int> &Rule);
    // 用于插入 label-reg 对
    void InsertPhi(std::pair<Operand, Operand> p) { this->phi_list.push_back(p); }
    // 返回结果寄存器号
    int GetResultRegNo() { return ((RegOperand *)result)->GetRegNo(); }
...
public:
    ...
    std::vector<Operand> GetUses() { return std::vector<Operand>(); }
    Operand GetModified() { return result; }
};
...
public:
    ...
    // 新加函数：重新设置 TrueLabel 和 FalseLabel
    void SetTrueLabel(Operand truelabel) { this->trueLabel = truelabel; }
    void SetFalseLabel(Operand falselabel) { this->falseLabel = falselabel; }
    ...
    Operand GetModified() { return nullptr; }
};
...
public:
    ...
    // 新加函数：重新设置 destlabel
    void SetDstLabel(Operand deslabel) { this->destLabel = deslabel; }
